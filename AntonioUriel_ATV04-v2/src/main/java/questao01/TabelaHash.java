...
public class TabelaHash <K,E,T> {
    ...
    /**
     * Processa uma chave, convertendo cada caractere para seu valor ascii e
     * aplicando um cálculo.
     * 
    ...
    private long converteChave(K chave){
        String chaveS = chave.toString();
        long chaveConvertida = 0;
        for (char character : chaveS.toCharArray()) {
            chaveConvertida += (long)character ;
        }    
        return (long) Math.pow(chaveConvertida *7,3) ;
    }
