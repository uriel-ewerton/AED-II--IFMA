...
public class Run {
     ...
     * 
     * <p>Insere dados a partir de um arquivo de entrada(entradaQ3.txt) em uma 
     * tabela hash, da classe TabelaHash. Em seguida lê e executa operações a 
     * partir de outro arquivo(exec.txt). 
     * 
     * <p>Os dados de entrada seguem o padrão "id;nome;email;cidade", sempre 
     * separado por ";". 
     * Exemplo: 12345678; João Silva; joaosilva@email.com; São Paulo
     * 
     * <p>Os dados dos arquivos foram retirados dos sites "https://mockaroo.com/"
     * e "https://www.invertexto.com/numeros-aleatorios", para criar uma
     * situação sem entradas repetitivas. "entradaQ3.txt" possui 50 entradas e
     * "entradaQ3_1.txt" possui 950.
     * 
     * <p>As operações sobre os dados armazenados, feitas a partir do arquivo 
     ...
     * 
     * <p>As funções testadas até o momento são:
     * "inserir", "remover", "buscar" e "imprimir".
     * Somente "imprimir", não necessita o uso do padrão.
     * 
    ...
    public static void main (String[] args){
        ...
        try (BufferedReader arquivo = new BufferedReader(new FileReader("src/entrada/entradaQ3.txt"))) {
            String linha;
        ...
        try (BufferedReader arquivo = new BufferedReader(new FileReader("src/entrada/exec.txt"))) {
            String linha;
            while((linha = arquivo.readLine()) != null){
                String[] campos = linha.split(";");
                System.out.println("__Função solicidada: " + campos[0].trim().toLowerCase());
                switch (campos.length) {
                    ...
                    case 5:
                        ...
                        switch(campos[0].trim().toLowerCase()){
                            case "inserir" -> t.inserir(cliente.getId(), cliente);
                            case "remover" -> t.remover(cliente.getId(), cliente);
                            case "buscar" -> t.buscar(cliente.getId(), cliente);
                        }   break;
                    default:
